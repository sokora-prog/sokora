...
export default function VoyageSearchScreen({ navigation, clientToken }) {
  ...
  const handleSearch = async () => {
    if (!origin.trim() || !destination.trim()) {
      Alert.alert('Champs requis', 'Veuillez saisir le départ et la destination.');
      return;
    }
    setLoading(true);
    setResults(null);
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.header}>
        <Text style={styles.title}>🚌 Réserver un voyage</Text>
          ...
          {results.length === 0 ? (
            ...
            results.map(trip => (
              <TouchableOpacity
                ...
                onPress={() => navigation?.navigate('SeatPicker', { trip, clientToken })}
              >
